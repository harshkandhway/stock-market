...
def run_backtest():
    ...
    for symbol in SYMBOLS:
        ...
        print(f"Backtesting {symbol} over the last {BACKTEST_DAYS} trading days (Option C: 1.5R Scale-Out)...")
        ...
        for i in range(start_idx, len(df) - 1):
            ...
            if in_trade:
                ...
                # Check Scale-Out (50% out at 1.5R)
                if not scaled_out and next_row['high'] >= scale_out_price:
                    exit_p = scale_out_price * (1 - SLIPPAGE_PCT)
                    # We do NOT log the trade here, we just bank the profit
                    # calculate the profit on the scaled-out portion
                    scaled_out = True
                    remaining_qty_ratio = 0.5
                    scale_out_executed_price = exit_p
                    continue
                    
                # Check Target (Remaining quantity)
                if next_row['high'] >= target:
                    exit_p = target * (1 - SLIPPAGE_PCT)
                    
                    # IF we scaled out previously, blend the exit prices
                    if scaled_out:
                        blended_exit = (scale_out_executed_price + exit_p) / 2
                        reason = "Target Hit (Scaled Out)"
                    else:
                        blended_exit = exit_p
                        reason = "Target Hit"
                        
                    trade = record_trade(symbol, entry_time, next_date, entry_price, blended_exit, reason)
                    if trade: all_trades.append(trade)
                    in_trade = False
                    continue
                    
                # Check Stop Loss first (worst case execution) - Remaining quantity
                if next_row['low'] <= stop_loss:
                    exit_p = stop_loss * (1 - SLIPPAGE_PCT)
                    
                    if scaled_out:
                        blended_exit = (scale_out_executed_price + exit_p) / 2
                        reason = "Stop Loss (Scaled Out)"
                    else:
                        blended_exit = exit_p
                        reason = "Stop Loss"
                ...
                if days_in_trade >= current_time_stop:
                    ...
                    if scaled_out:
                        blended_exit = (scale_out_executed_price + exit_p) / 2
                        reason = f"Time Stop ({current_time_stop}d - Scaled Out)"
                    else:
                        blended_exit = exit_p
                        reason = f"Time Stop ({current_time_stop} days)"
            ...
            if not in_trade:
                ...
                try:
                    ...
                    if ("STRONG BUY" in rec or "BUY" == rec) and ("AVOID" not in rec and "WARNING" not in rec) and conf >= 70:
                        ...
                        # Initialize scale-out
                        scaled_out = False
                        remaining_qty_ratio = 1.0
                        initial_risk = entry_price - stop_loss
                        scale_out_price = entry_price + (initial_risk * 1.5)
                except Exception as e:
        ...
        if in_trade:
            ...
            if scaled_out:
                blended_exit = (scale_out_executed_price + exit_p) / 2
                reason = "End of Backtest (Scaled Out)"
            else:
                blended_exit = exit_p
                reason = "End of Backtest"
                
            trade = record_trade(symbol, entry_time, last_date, entry_price, blended_exit, reason)
            if trade: all_trades.append(trade)
