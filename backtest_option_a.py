...
def get_earnings_dates(symbol: str) -> list:
    ...
    try:
        ...
        dates = []
        if isinstance(cal, dict) and symbol in cal and 'earnings' in cal[symbol]:
            earnings_obj = cal[symbol]['earnings']
            if 'earningsDate' in earnings_obj:
                for date_str in earnings_obj['earningsDate']:
                    try:
                        # "2026-04-24 15:30:S" format
                        dt = datetime.strptime(date_str[:10], '%Y-%m-%d').date()
                        dates.append(dt)
                    except:
                        pass
        dates.sort()
        earnings_cache[symbol] = dates
        return dates
    except Exception as e:
...
def run_backtest():
    ...
    for symbol in SYMBOLS:
        ...
        print(f"Backtesting {symbol} over the last {BACKTEST_DAYS} trading days (Option A: Earnings Blackout)...")
        
        # Prefetch earnings dates
        all_earnings_dates = get_earnings_dates(symbol)
            ...
            if not in_trade:
                ...
                try:
                    ...
                    if ("STRONG BUY" in rec or "BUY" == rec) and ("AVOID" not in rec and "WARNING" not in rec) and conf >= 70:
                        # OPTION A: Binary Event Blackout (Earnings Filter)
                        # Ensure we are NOT within 7 days of the NEXT earnings date
                        is_blacked_out = False
                        if all_earnings_dates:
                            # find the next earnings date after curr_date
                            future_earnings = [d for d in all_earnings_dates if d >= curr_date.date()]
                            if future_earnings:
                                next_earnings = future_earnings[0]
                                days_to_earnings = (next_earnings - curr_date.date()).days
                                if days_to_earnings <= 7:
                                    is_blacked_out = True
                        
                        if is_blacked_out:
                            # Skip this trade, binary event risk is too high
                            continue
