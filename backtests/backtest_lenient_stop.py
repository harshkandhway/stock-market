#!/usr/bin/env python3
"""
Stock Analyzer Pro Backtester — Lenient Stop (5% Buffer)
========================================================
Strategy: V1 Baseline + 5% Lenient Stop Loss
- Entry: STRONG BUY / BUY (conf >= 70) → Buy at next day's open
- Stop: Engine stop_loss * 0.95 (5% buffer)
- Target: Engine target
- Time Stop: Hard 63-day exit (no extensions)
- Result: ~₹191k Net P&L, ~56% Win Rate
"""
import sys
import os
import pandas as pd
...
def run_backtest():
    ...
    for symbol in SYMBOLS:
        ...
        print(f"Backtesting {symbol} (Lenient Stop)...")
            ...
            if not in_trade:
                ...
                try:
                    ...
                    if ("STRONG BUY" in rec or "BUY" == rec) and ("AVOID" not in rec and "WARNING" not in rec) and conf >= 70:
                        ...
                        stop_loss = analysis['stop_loss'] * 0.95  # 5% lenient buffer
                        days_in_trade = 0
                except Exception:
                    pass
    ...
    print("\n" + "="*50)
    print("🎯 BACKTEST RESULTS — LENIENT STOP (5% BUFFER)")
    print("="*50)
    ...
    trades_df.to_csv('backtest_lenient_stop_trades.csv', index=False)
    print("\nDetailed trades saved to 'backtest_lenient_stop_trades.csv'")
    print("\nBreakdown by Reason:")
