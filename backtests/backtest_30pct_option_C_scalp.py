#!/usr/bin/env python3
"""
OPTION C: THE BREAKOUT SCALPER (30% Yield Pursuit)
==================================================
Shift from 60-day swing trading to 5-day rapid momentum scalping.
Goal: Increase capital velocity. Reinvest cash every 3-5 days.

Rules:
- Entry: Conf>=60%, RSI 40-65, Volume > 20d avg, Price > EMA-21
- Hard Target: +3.0%
- Hard Stop Loss: -1.5%
- Time Stop: 5 Days (Exit regardless of P&L)
"""
...
HORIZON_DAYS = 5  # Rapid time stop
TARGET_PCT = 0.03
STOP_PCT = 0.015
...
def position_size(confidence, capital):
    """Slightly aggressive sizing since we exit in 5 days."""
    if confidence >= 80: return 50000
    elif confidence >= 70: return 30000
    else: return 15000
...
def run_backtest():
    ...
    print("=" * 65)
    print(f"OPTION C: BREAKOUT SCALPER ({len(universe)} STOCKS)")
    print("=" * 65)
    ...
    for date_idx, curr_date in enumerate(trading_dates):
        ...
        for sym, pos in open_positions.items():
            ...
            if nr['low'] <= pos['stop_loss']:
                xp = pos['stop_loss'] * (1 - SLIPPAGE_PCT)
                ch = calc_costs(pos['entry_price'], xp, pos['qty'])
                g = (xp - pos['entry_price']) * pos['qty']; n = g - ch
                cash += (pos['entry_price'] * pos['qty']) + n
                closed_trades.append({'symbol': sym, 'entry_time': pos['entry_time'], 'exit_time': next_date, 'days_held': pos['days'], 'entry_price': pos['entry_price'], 'exit_price': xp, 'qty': pos['qty'], 'invested': pos['entry_price']*pos['qty'], 'reason': 'Stop Loss', 'net_pnl': n, 'win': 0})
                to_close.append(sym)
                continue
            ...
            if nr['high'] >= pos['target']:
                ...
                g = (xp - pos['entry_price']) * pos['qty']; n = g - ch
                cash += (pos['entry_price'] * pos['qty']) + n
                closed_trades.append({'symbol': sym, 'entry_time': pos['entry_time'], 'exit_time': next_date, 'days_held': pos['days'], 'entry_price': pos['entry_price'], 'exit_price': xp, 'qty': pos['qty'], 'invested': pos['entry_price']*pos['qty'], 'reason': 'Target Hit', 'net_pnl': n, 'win': 1})
                to_close.append(sym)
                continue
    ...
    pnl = tdf['net_pnl'].sum()
    print(f"\n💰 C: BREAKOUT SCALPER | RETURN: ₹{pnl:+,.0f} | TRADES: {len(tdf)} | WIN RATE: {len(tdf[tdf['win']==1])/len(tdf)*100:.1f}%")
    tdf.to_csv('option_c_trades.csv', index=False)
