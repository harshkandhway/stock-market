...
def fetch_data(symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        # ATR Contraction (Option D)
        df['atr'] = ta.volatility.AverageTrueRange(high=df['high'], low=df['low'], close=df['close'], window=14).average_true_range()
        # Calculate trailing 6-month (126 trading days) ATR percentile rank
        df['atr_rank_126d'] = df['atr'].rolling(window=126).apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False)
        
        # Format the df to match what analyze_stock expects
        return df[['open', 'high', 'low', 'close', 'volume', 'ema_21', 'atr', 'atr_rank_126d']]
...
def run_backtest():
    ...
    for symbol in SYMBOLS:
        ...
        print(f"Backtesting {symbol} over the last {BACKTEST_DAYS} trading days (Option D: Coiled Spring)...")
            ...
            if not in_trade:
                ...
                try:
                    ...
                    if ("STRONG BUY" in rec or "BUY" == rec) and ("AVOID" not in rec and "WARNING" not in rec) and conf >= 70:
                        ...
                        # OPTION D: Coiled Spring (Volatility Contraction)
                        # The stock must be emerging from a period of historical silence.
                        # That means its 14-day ATR should be in the BOTTOM 25% of its 6-month volatility range
                        is_coiled = False
                        if not pd.isna(curr_row['atr_rank_126d']):
                            if curr_row['atr_rank_126d'] <= 0.25:
                                is_coiled = True
                        else:
                            # if no 6M historical data, assume it's coiled
                            is_coiled = True
                            
                        if not is_coiled:
                            continue # Skip the trade, momentum is chasing, not breaking out
